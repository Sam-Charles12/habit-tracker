...
import {
  ...
  toggleHabitCompletionInStorage,
  calculateStreak,
} from "@/lib/habits";
...
export default function DashboardPage() {
  ...
  const handleToggleHabit = (habitId: string) => {
    if (!user) return;
    toggleHabitCompletionInStorage(habitId, todayStr);
    refreshHabits(user.id);
  };
  ...
  return (
    <main
      ...
      className="flex-grow bg-zinc-950 text-zinc-100 p-4 pt-16 md:p-8 md:pt-16 min-h-screen relative pb-24"
    >
      <div className="max-w-xl mx-auto">
        ...
        <div className="mb-4 flex justify-between items-center">
          ...
          <span className="text-sm text-zinc-500 cursor-pointer hover:text-emerald-400">
            See all
          </span>
        </div>
          ...
          <div className="space-y-4">
            {habits.length === 0 ? (
              <div
                ...
                className="text-center py-10 bg-zinc-900/50 rounded-3xl border border-zinc-800/50 relative z-10 ml-12"
              >
                  ...
                  >
                    ...
                    <div className="bg-zinc-900 rounded-[1.5rem] p-3 flex-grow flex items-center justify-between border border-zinc-800/80 shadow-sm transition-all hover:border-zinc-700">
                      <div className="flex items-center gap-4 pl-1">
                        ...
                        <div>
                          <h3
                            className={`font-medium ${isCompleted ? "text-zinc-500 line-through" : "text-zinc-100"}`}
                          >
                            {habit.name}
                          </h3>
                          <p
                      ...
                      <div className="flex items-center">
                        ...
                        <button
                          ...
                          className="ml-2 text-zinc-600 hover:text-emerald-400 transition-opacity"
                          title="Edit"
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                        ...
                        <button
                          ...
                          className="ml-2 text-zinc-600 hover:text-red-400 transition-opacity"
                          title="Delete"
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
            ...
            <div className="bg-zinc-900 rounded-2xl p-6 max-w-sm w-full">
              ...
              <div className="flex justify-end gap-3">
                <button
                  ...
                  className="px-4 py-2 rounded bg-zinc-800 text-zinc-300"
                >
                  Cancel
                </button>
                <button
    ...
    <div className="fixed inset-0 bg-zinc-950 z-50 overflow-y-auto flex flex-col p-6 animate-in slide-in-from-bottom-full duration-300">
      ...
      <div className="flex justify-center mb-10">
        <div className="w-48 h-48 relative flex items-center justify-center">
          <div className="w-32 h-32 bg-emerald-500 rounded-3xl border-4 border-zinc-950 flex flex-col items-center pt-6 pb-4 px-4" />
        </div>
      </div>
      ...
      <form
        ...
        className="space-y-8 flex-grow max-w-md mx-auto w-full"
      >
        <div>
        ...
        <div>
          <label className="block text-zinc-400 text-sm mb-2">Frequency</label>
          <select
        ...
        <div className="mt-auto pt-6 max-w-md mx-auto w-full">
          <button
            ...
            type="submit"
            disabled={!title.trim()}
            className="w-full py-4 rounded-full bg-orange-500 text-zinc-50 font-bold text-lg hover:bg-orange-400 disabled:opacity-50 transition-colors shadow-lg shadow-orange-500/20"
          >
            Save Habit
          </button>
        </div>
...
function EditHabitModal({
  ...
  return (
    <div className="fixed inset-0 bg-zinc-950 z-50 overflow-y-auto flex flex-col p-6 animate-in slide-in-from-bottom-full duration-300">
      {/* Header */}
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold text-zinc-100">Edit habit</h1>
        <button
          onClick={onClose}
          className="w-10 h-10 bg-zinc-900 border border-zinc-800 rounded-full flex items-center justify-center text-zinc-300 hover:text-zinc-100 transition-colors"
        >
      ...
      <div className="flex justify-center mb-10">
        <div className="w-48 h-48 relative flex items-center justify-center">
          ...
          <svg
            className="absolute top-4 left-4 w-8 h-8 text-orange-400"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="3"
          ...
          <div className="w-32 h-32 bg-emerald-500 rounded-3xl border-4 border-zinc-950 shadow-[0_0_0_4px_rgba(16,185,129,1)] flex flex-col items-center pt-6 pb-4 px-4">
            ...
            <div className="bg-zinc-100 w-full flex-grow rounded-xl border-4 border-zinc-950 grid grid-cols-4 grid-rows-2 gap-1 p-1">
              {[...Array(8)].map((_, i) => (
                <div
                  key={i}
                  className={`border-2 border-zinc-950 rounded-sm ${i === 6 ? "bg-emerald-400" : "bg-transparent"}`}
                ></div>
              ))}
            </div>
          </div>
      ...
      <div className="space-y-8 flex-grow max-w-md mx-auto w-full">
        ...
        {/* Goal */}
        <div
          className={`transition-opacity ${!goalChecked ? "opacity-50" : "opacity-100"}`}
        >
          <div
            className="flex justify-between items-center mb-2 cursor-pointer"
            onClick={() => setGoalChecked(!goalChecked)}
          >
            <label className="text-zinc-400 text-sm cursor-pointer pointer-events-none">
              Set a goal
            </label>
            <div
          ...
          <div className="flex gap-4">
            <div className="flex-1 bg-zinc-900 border border-zinc-800 rounded-2xl px-5 py-4 flex justify-between items-center relative overflow-hidden focus-within:ring-2 focus-within:ring-emerald-500/50">
              ...
              />
              {!goalDate && (
                <span className="absolute left-5 text-zinc-500 pointer-events-none">
                  Add date
                </span>
              )}
              <svg
                className="w-5 h-5 text-zinc-500 pointer-events-none absolute right-5"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
                strokeWidth="2"
            ...
            <div className="flex-1 bg-zinc-900 border border-zinc-800 rounded-2xl px-5 py-4 flex justify-between items-center focus-within:ring-2 focus-within:ring-emerald-500/50 relative">
              ...
              />
              <svg
                className="w-5 h-5 text-zinc-500 pointer-events-none"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
                strokeWidth="2"
        ...
        {/* Repeat Days */}
        <div
          className={`transition-opacity ${!repeatChecked ? "opacity-50" : "opacity-100"}`}
        >
          <div
            className="flex justify-between items-center mb-4 cursor-pointer"
            onClick={() => setRepeatChecked(!repeatChecked)}
          >
            <label className="text-zinc-400 text-sm cursor-pointer pointer-events-none">
              Repeat days
            </label>
